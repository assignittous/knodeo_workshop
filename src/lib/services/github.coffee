...
currentRepo.show (err, repo)->
  fs.writeFileSync("#{data_dir}/#{datestamp}_repos.csv", convert.arrayToCsv([repo], attributes.repo)) 
...
#currentRepo.getCommits {since: "2008-01-01"}, (err, commits)->
#  console.log "Got #{commits.length} for repo: #{currentRepo.full_name} since 2008-01-01"


#issues = github.getIssues('assignit','pi_app')

#issues.list {"state": "closed"}, (err,data)->
